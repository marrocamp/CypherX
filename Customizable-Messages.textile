...
{PLUGIN} - Plugin name (xAuth)
{VERSION} - Current version of this plugin

{PLAYER} - Name of the player the message is being sent to
{IP} - IP address of the player the message is being sent to

{TARGET} - Name of the target player in admin commands
{TARGETIP} - IP address of the target player in admin commands

{PWMINLENGTH} - Password minimum length
{NAMEMINLENGTH} - Name minimum length
{MAXSTRIKES} - Amount of strikes (invalid password) attempts

{NEWLINE} - Split a message into multiple lines
</code></pre>